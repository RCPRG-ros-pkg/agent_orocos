...
#        eprint("   " + n)
    ...
    for n in children:
        ...
        s = StringIO()
        s.write("# autogenerated by rtt_subsystem_buffers/create_msgs_from_ec_config.py\n")
    ...
    if debug_ec_data:
        ...
        variables_sorted = []
        for v in variables:
            variables_sorted.append( (v.Name, v.DataType) )

        variables_sorted = sorted(variables_sorted, key=lambda x: x[0])
        for v in variables_sorted:
            dbg_out += v[1] + '    ' + v[0] + '\n'

        with open('/tmp/dbg_' + top_level_name + '.txt', 'w') as dbg_f:
            dbg_f.write(dbg_out)
